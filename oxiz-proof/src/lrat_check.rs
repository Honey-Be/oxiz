//! PORTED VERBATIM from upstream oxiz `v0.3.3`
//! (`oxiz-proof/src/lrat_check.rs`), same Apache-2.0 licence, unmodified.
//!
//! Why this file and not the generator alongside it: upstream's LRAT
//! PRODUCER (`oxiz-sat/src/solver/lrat_trace.rs` plus hooks threaded through
//! `add_clause.rs`/`proof.rs`) lives in exactly the code our fork rewrote in
//! the §4 SAT-core redesign, so porting it is a merge job, not a copy. The
//! CHECKER depends on nothing but `std` — it takes a clause list and a text
//! LRAT stream — so it ports as a file and is useful immediately: it lets
//! adsmt CHECK an unsat certificate no matter who produced it, including an
//! external LRAT producer, which is the first rung of
//! `adsmt-delegate/DELEGATION_TRUST_REDESIGN.md` §S2 ("a delegated `unsat`
//! must EARN `DefiniteUnsat`"). Keeping it byte-identical to upstream is
//! deliberate: it makes the next upstream sync a no-op for this file, and a
//! proof checker is the last place to want a local variant.
//!
//! ONE deviation, confined to the test module: upstream's temp-directory names
//! call `oxiz_time::SystemTime`, a shim crate this fork does not vendor. Those
//! four references are `std::time::SystemTime` here. The CHECKER itself — every
//! line that decides whether a proof verifies — is untouched.
