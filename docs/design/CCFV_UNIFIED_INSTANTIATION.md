...
| **P2b** ✅ **LANDED** (2026-06-20, opt-in driver wiring) | Driver wiring landed: `SolverConfig::ccfv_ematch` (new); `(set-option :oxiz.ccfv-ematch true)` SMT-LIB switch (`context.rs`); `mod.rs` builds the engine with the flag and, at the round call, `EufCongruence::new(&self.euf)` (the live post-solve congruence, restored by `restore_theory_manager`) → `round_with_cong`. **ccfv-on end-to-end smoke** green: `:oxiz.ccfv-ematch true` + `∀a.f(a)=a :pat(f a)` derives `f(3)=4 → unsat` and the consistent `f(3)=3 → sat` through the full live path. | med |
| **P2-flip** ✅ **LANDED** (2026-06-20, default ON; `9a84198`) | `SolverConfig::ccfv_ematch` default flipped `false`→`true` in all 4 presets. **Both default-flip gates passed:** (1) z3-parity corpus under `OXIZ_CCFV_EMATCH=1` is **byte-identical** to syntactic (166/168 agree, 0 spurious — no corpus case has a nested-congruence trigger, so CCFV changes no verdict); (2) full-prelude consult shows **0 measurable overhead** (CCFV off 103/101/101 ms vs on 105/101/106 ms — most congruence classes are singletons). The flip is also a **soundness repair**: on the constructed (z3-confirmed) shape `∀x.ensL(bx x)=(x>5) :pat((ensL (bx x)))` + `c=bx(yc) ∧ ensL(c) ∧ ¬(yc>5)`, CCFV-off certifies a congruence-blind model as spurious `sat`; CCFV-on fires the trigger modulo `c=bx(yc)` and derives the genuine `unsat`. Locked by regression `nested_congruence_trigger_is_sound`. | med |
| **match_term del** ✅ **LANDED** (2026-06-20, `cd573ce`) | The syntactic `trigger` module (`match_single`/`match_multi`/`match_term`) was the dead default fallback — deleted. **CCFV is now the SOLE matcher**: `engine::ematch_all` routes single- AND multi-pattern groups through `ccfv::match_trigger`/new `ccfv::match_trigger_multi`, the `ccfv_ematch` flag selecting the real-congruence oracle vs the syntactic-equivalent `NoCong`. `match_trigger_multi` also fixes a latent limitation — the old `match_multi` required the FIRST pattern to bind every var, so a genuine `((f x) (g y))` matched nothing; the join now completes partial bindings across patterns. Verdict-preserving: corpus byte-identical (166/1/1, 0 spurious); +4 multi-pattern unit tests. | med |
| **P3** ✅ **LANDED** (2026-06-20, `f8b9582`) | `cdqi::find_conflict` → `find_conflicts`, taking the [`Congruence`] oracle: (1) congruence-deduped candidate domains (`cong.rep`, one INDEX term per class — fewer redundant body evals, the (a) lookup; `NoCong` ⇒ identity ⇒ old odometer) and (2) **all** conflicts not the first (CCFV `C=¬ψ`), emitted together. Verdict-preserving refactor (each conflict still a ground-tuple instance through the unchanged firewall): corpus byte-identical (166/1/1, 0 spurious); #229 regressions hold. | med |
| **P4 (scaffolding)** ✅ **LANDED** (2026-06-20) | The model-completion STRUCTURE lands as a **pure addition** (no live consumer, like P0/P1): `congruence::Mode` (the §3 `Trigger`/`Conflict`/`ModelCompl` parameterization, made explicit) + `congruence::TotalView` (`E_TOT` = base congruence `+ ξ_f` default values via [`Congruence::default_value`], `disequal = !equal`). `rep`/`equal`/`class`/`apps_like` delegate to the base. +3 unit tests; corpus byte-identical. |
| **P4 Phase 1+2** ✅ **LANDED** (2026-06-20) | The complete brute-force `solve` conflict search (`ccfv::solve` over `class_reps_of_sort`, a SUPERSET of conflicts — keep σ unless a literal is decidably `Some(false)`, so empty ⇒ no conflict) + the EUF disequality reader (`are_disequal_immutable`) + the `Congruence` `disequal`/`ground_of_sort`/`class_reps_of_sort` witness-domain surface. The **brute-force keystone**: completeness holds **by construction** (no pruning ⇒ no `fail_sound` proof needed), verus-verified in `ccfv-verification/src/complete.rs` (`no_conflict_when_empty`). |
| **P4 Phase 3** ✅ **LANDED** (2026-06-20, flag-OFF) — verdict-flip WIRED | The (b) verdict-flip goes LIVE behind `SolverConfig::ccfv_model_compl` (**default false**; `(set-option :oxiz.ccfv-model-compl true)`). `ModelEval::model_completion<C: Congruence>(lang, cong, quant)` is the engine's backstop after `eval_forall` returns `None` (consulted at the mid-round short-circuit AND the saturation verdict, both gated): the host lowers `¬ψ` to a (dis)equality DNF (`negate_body_to_dnf`), passes the §6 **accounting gate** (every completion-target symbol's quantifier-scoped occurrences are exactly this body's — `under_quant_occ == body_occ`; ground occurrences are interned in `E` and read at their true value by the search), then runs `solve` over a `TotalView` of the live `EufCongruence` (carrying the by-sort witness index). EMPTY ⇒ `Some(true)` (Sat); any conflict / undecidable lowering / unmet gate / empty-or-over-budget domain ⇒ `None` (never `Some(false)`). **Restricted to the pure uninterpreted-sort E-(dis)unification fragment**: Bool predicates (2-valued sort — free-distinct completion unsound) and Int/Real/BV/array/datatype equalities (theory could force an unseen merge) DECLINE. +6 regression tests incl. the soundness bars (unsat / arith-forced / contradictory-predicate / two-quantifier all stay non-`sat`); flag-off path byte-identical. | high (soundness) |
| **P4 Phase 4** ✅ **LANDED** (2026-06-21, `[선검증]`) | The **conservative-extension / fresh-element** verus proof — the obligation `complete.rs` left open and `diseq::flip_is_sound` named. New `ccfv-verification/src/model_compl.rs`: `recolor_preserves_dmodels` (recoloring a term the context mentions nowhere preserves every model — the completion may assign a fresh application any value) + `fresh_imposes_no_conflict` (a FRESH element is entailed NEITHER equal nor disequal to any distinct term, so a `¬ψ` cube binding a fresh element is never entailed). Combined with `complete::no_conflict_when_empty` (domain-completeness), the flip's "search ∅ ⇒ `Sat`" is sound over the WHOLE infinite uninterpreted sort, not merely the enumerated reps. **41 verified, 0 errors** (was 37). The flip's soundness is now FULLY pre-verified; the only remaining gate to flipping the flag on by default is the empirical corpus 0-spurious sweep (Phase 5), not a proof. | high (soundness) |
...
Each phase is independently shippable and independently revertible. P0–P3 +
match_term-del are **refactors** (same verdicts, less code, congruence-correct);
P4 lands the (b) completeness *structure* with the verdict-flip gated on complete
CCFV + verus; P5 is the (a) performance win.
