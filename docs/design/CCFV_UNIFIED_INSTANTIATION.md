...
| **P2b** (pending — the live change + corpus gate) | Driver wiring: `oxiz-solver/.../mod.rs` builds an `EufCongruence` and calls `round_with_cong` with `ccfv_ematch` on; flip the default after the **`clean_mbqi_corpus --ignored`** gate confirms no spurious verdicts + matches ≥ old (user `!` gate). Then delete `trigger::match_term`. | med |
