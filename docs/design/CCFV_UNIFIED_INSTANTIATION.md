...
| **P2** | Re-express **trigger e-matching** as `ccfv::solve(Ground, ⋀pᵢ≃yᵢ)` + discard. Delete `trigger::match_term`. Validate: corpus + prelude unchanged verdicts, matches ≥ old (congruence adds matches). | med |
